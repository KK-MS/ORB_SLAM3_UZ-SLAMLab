...
#endif
    thread threadLeft(&Frame::ExtractORB,this,0,imLeft,0,0, mask_left);
    thread threadRight(&Frame::ExtractORB,this,1,imRight,0,0, mask_right);
    threadLeft.join();
    threadRight.join();
#ifdef REGISTER_TIMES
...
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera, const cv::Mat &mask, Frame* pPrevF, const IMU::Calib &ImuCalib)
...
void Frame::ExtractORB(int flag, const cv::Mat &im, const int x0, const int x1, const cv::Mat &mask)
{
    ...
    cv::Mat image(1080, 1920, CV_8UC3, cv::Scalar(0));
    if(flag==0){
        monoLeft = (*mpORBextractorLeft)(im,mask,mvKeys,mDescriptors,vLapping);
        for (const auto& keypoint : mvKeys) {
            cv::circle(image,cv::Point(keypoint.pt.x, keypoint.pt.y), 10, cv::Scalar(0, 255, 0));
        }
    }
    else {
        monoRight = (*mpORBextractorRight)(im,mask,mvKeysRight,mDescriptorsRight,vLapping);
        for (const auto& keypoint : mvKeysRight) {
            cv::circle(image,cv::Point(keypoint.pt.x, keypoint.pt.y), 10, cv::Scalar(0, 255, 0));
        }
    }
...
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timeStamp, ORBextractor* extractorLeft, ORBextractor* extractorRight, ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera, GeometricCamera* pCamera2, Sophus::SE3f& Tlr,const cv::Mat &mask_left, const cv::Mat &mask_right, Frame* pPrevF, const IMU::Calib &ImuCalib)
        :mpcpi(NULL), mpORBvocabulary(voc),mpORBextractorLeft(extractorLeft),mpORBextractorRight(extractorRight), mTimeStamp(timeStamp), mK(K.clone()), mK_(Converter::toMatrix3f(K)),  mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth),
...

{
    ...
    std::chrono::steady_clock::time_point time_StartExtORB = std::chrono::steady_clock::now();
#endif
    thread threadLeft(&Frame::ExtractORB,this,0,imLeft,static_cast<KannalaBrandt8*>(mpCamera)->mvLappingArea[0],static_cast<KannalaBrandt8*>(mpCamera)->mvLappingArea[1], mask_left, mask_right);
